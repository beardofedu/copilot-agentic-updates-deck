...
| 0:22–0:31 Defender + Code Security | `public/app.js` | [#8 Fix reflected XSS in promo banner](https://github.com/beardofedu/copilot-agentic-updates-deck/issues/8) | **Security remediation narrative:** Show the XSS vulnerability (innerHTML with unsanitized query param), the Copilot-assisted remediation PR, and the fix pattern (textContent + input length limits). |
